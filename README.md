...
- **Session State** — The dot beside each session answers one question: does this session still want something from you? One of four states, one at a time — a grey ring when it's sleeping, a blue ring that breathes while it's working, a yellow dotted ring when it needs your input, and a solid green disc when it's done. `done` is declared, not guessed: mark it yourself in a click, or let the agent close its own session with an MCP tool; it lifts automatically the moment work resumes and survives a restart. Plain and Run Terminals drop to a single compact line with no state of their own.
