...
- **Session State** — The dot on each session row answers one question at a glance: is this session waiting on you? One of four states shows at a time — a grey ring (sleeping), a breathing blue ring (working), a dashed amber ring (needs your input), or a solid green dot (done). `done` is declared, never guessed: mark it yourself from the row, or let the agent call it through the `markSessionDone` MCP tool — and it lifts itself the moment the session gets back to work. Terminals drop to a single compact line, and the "running" filter now shows the sessions that are actually active.
