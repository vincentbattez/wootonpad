...
test('a live PTY with no activity is sleeping, never a state of its own', () => {
  const state = sessionStateFor({ isRunning: true });
  assert.equal(state, 'sleeping');
  assert.ok(SESSION_STATES.includes(state));
});

test('a live PTY does not colour a done or a working row either', () => {
  assert.equal(sessionStateFor({ isRunning: true, done: true }), 'done');
  assert.equal(sessionStateFor({ isRunning: true, isBusy: true }), 'working');
});
...
test('a terminal-like row has no Session State whatever the signals', () => {
  for (const type of ['terminal', 'run-terminal']) {
    ...
    assert.equal(sessionStateFor({ type, isRunning: true }), null);
  }
});
...
test('every state maps to exactly one dot class, and nothing else does', () => {
  ...
  for (const c of classes) assert.match(c, /^session-status-dot--[a-z-]+$/);
