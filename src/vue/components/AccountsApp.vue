...
// The list, active account and usage come from the feature store the accounts
// bridge writes; the edit/add form state below stays component-local.
...
// setAccounts used to kick off WSL-home discovery; the store write now lands in
// the bridge, so the discovery moves to a watcher. loadWslHomes is idempotent,
// so firing on the first list is enough.
watch(() => accountsStore.accounts, () => loadWslHomes());
