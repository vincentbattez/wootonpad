...
export async function setAreaImageFromFile(areaId, file) {
  if (!areaId || !file || !window.api?.setAreaImage) return null;
  const path = window.api.getPathForFile?.(file);
  if (!path) return null;
  const url = await window.api.setAreaImage(areaId, path).catch(() => null);
...
export async function clearAreaImage(areaId) {
  if (!areaId || !window.api?.clearAreaImage) return;
  await window.api.clearAreaImage(areaId).catch(() => {});
